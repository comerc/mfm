# Соглашения по тестированию

- [testing/synctest experiment for time and concurrency testing](https://danp.net/posts/synctest-experiment/)
- Применяется `assert.AnError` вместо `errors.New("some error")`
- Применяется `t.Parallel()` и флаг для выявления гонки данных
- Для call-тестов (`func(t *testing.T)`) тоже применяется `t.Parallel()` 
- Применяется AAA (Arange Act Asset), т.е. assert-ы для проверки условий, а request-для проверки сетапа
- Юнит-тесты размещаются в том же пакете, что и тестируемый код, с суффиксом `_test.go`
- Интеграционные тесты размещаются в директории `/test`
- Приёмочные тесты размещаются в директории `/test/e2e`
- Для каждой функциональной единицы должны быть написаны тесты, покрывающие основные сценарии использования
- Покрытие юнит-тестами выполняется с конца тестируемого файла, т.е от общего к частному (при условии, что методы в файле расположены в порядке: публичные, непубличные, утилиты); в самом начале должен быть основной "success" тест.
- Имена юнит-тестов должны быть без имени тестируемого объекта: 
  - Test - если всё уместилось дочерние в табличные тесты
  - TestPublicMethod - если отдельный тест для публичного метода
  - Test_privateMethod - если отдельный тест для приватного метода
- Имена табличных тестов должны быть в snake_case, без пробелов и заглавных букв, допустимы дефисы (это облегчает поиск)
- Использовать `t.Cleanup()` для освобождения ресурсов (вместо `defer`)
